"""It won't get better than this version."""
...
def set_register_to(register: Register, value: int) -> str:
    ...
    # SMARTH TRACKING OF THIS IS HARD.
...
class Compiler:
    ...
    def set_value_to_register(self, register_obj: Register, value: language.Value) -> str:
        ...
        if isinstance(value, (language.Number, language.Variable)):
            if isinstance(value, language.Variable):
                # SUPER OPTIMIZED REG ONLY VALUE
                if value.register:
    ...
    def prepare(self):
        """Replace all variables with location in memory etc."""

        # Assign memory cell, and register if it's special variable.
        variables = list(self.manager.variables.values())
        for index in range(0, len(variables)):
            variables[index].memory_cell = index
            if variables[index].register:
                self.registers[variables[index].register].associate(self)
                self.registers[variables[index].register].change_content(self,
                                                                         variables[index])
            else:
                # No special optimizations.
                variables[index].register = False


        # Read about special variables from parse info.
...
def compile_command(something: language.Command, compiler: Compiler) -> MachineCode:
    ...
    elif isinstance(something, optimizer_stuff.SpecialCommand):
        block = special_stuff(something, compiler)
        return block
    else:
...
def special_stuff(something: optimizer_stuff.SpecialCommand, compiler: Compiler):
    """Do special stuff basing on command."""
    if isinstance(something, optimizer_stuff.VariableToRegisterStart):
        # Hello General Variable.
        variable = something.variable
        print_debug("GIVE TURBO TO: " + str(variable))
        free_reg = None
        for reg in compiler.registers:
            if not compiler.registers[reg].taken:
                free_reg = compiler.registers[reg]
                break
        if free_reg is not None:
            block = MachineCode()
            block += free_reg.associate(special_stuff)
            free_reg.change_content(special_stuff, variable)
            variable.register = free_reg.name
            return block
        else:
            print_debug("Can not convert variable to REG ONLY (NO FREE REGISTER)")
    elif isinstance(something, optimizer_stuff.DeleteRegister):
        # Hasta 'la Vista variable!
        print_debug("RELEASE REGISTER")
        variable = something.variable
        if variable.register:
            block = MachineCode()
            block += compiler.registers[variable.register].free(special_stuff)
            return block
        else:
            print_debug("Can not RELEASE REGISTER BECAUSE IT WAS NOT ASSIGNED AT ALL")
    elif isinstance(something, optimizer_stuff.RegisterToVariable):
        # Good 'ol memory
        print_debug("VARIABLE TO MEMORY")
        variable = something.variable
        if variable.register:
            block = MachineCode()
            address_reg = compiler.registers["A"]
            block += address_reg.force_associate(special_stuff)
            block.add_commands(set_register_to(address_reg,
                                              variable.memory_cell))
            block.add_command("STORE {0}".format(variable.register))
            block += address_reg.free(special_stuff)

            compiler.registers[variable.register].free(special_stuff)
            variable.register = False
            return block
        else:
            print_debug("Can not VARIABLE TO MEMORY BECAUSE IT WAS NOT ASSIGNED RLY")
    elif isinstance(something, optimizer_stuff.VariableToRegister):
        # You were meant to destroy the Siths, not join them!
        variable = something.variable
        print_debug("VARIABLE TO REGISTER: " + str(variable))
        free_reg = None
        for reg in compiler.registers:
            if not compiler.registers[reg].taken:
                free_reg = compiler.registers[reg]
        ...
        if free_reg is not None:
            ...
            block.add_commands(set_register_to(address_reg,
                                               variable.memory_cell))
            block.add_command("LOAD {0}".format(variable.register))
            block += address_reg.free(special_stuff)
...
class AssignCompiler:
    ...
    def compile(self) -> MachineCode:
        ...
        if isinstance(self.assign.identifier, language.Variable) and self.assign.identifier.register:
                # Just copy to register.

                block.add_command("COPY {0} {1}".format(self.assign.identifier.register,
                                              get_register.name))
...
class ReadCompiler:
    ...
    def compile(self) -> MachineCode:
        ...
        INDENTATION += 1
        block = MachineCode()
        if isinstance(self.read.identifier, language.Variable) and self.read.identifier.register:
            block.add_command("GET {0}".format(self.read.identifier.register))
            return block
